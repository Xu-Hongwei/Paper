...
def label_aware_copa_loss(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        if gate_type == "label_support":
            diff_tv = torch.abs(s_t - s_v)
            diff_ta = torch.abs(s_t - s_a)
            g_tv_agr = s_t * s_v * (1.0 - diff_tv)
            g_ta_agr = s_t * s_a * (1.0 - diff_ta)
            g_tv_comp = torch.maximum(s_t, s_v) * diff_tv
            g_ta_comp = torch.maximum(s_t, s_a) * diff_ta
        elif gate_type == "full_relation":
            ...
            raise ValueError("gate_type must be 'label_support' or 'full_relation'.")
